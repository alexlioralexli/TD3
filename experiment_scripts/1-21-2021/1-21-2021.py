# expID should start with 19

envs = ['Humanoid-v2', 'dm.finger.turn_hard', 'dm.acrobot.swingu    p', 'dm.acrobot.swingup_sparse']
for env in envs:
    commands = []
    for depth in ['--n_hidden 4']:
        commands.append(f'python main.py --policy SAC --automatic_entropy_tuning --env {env} --network_class D2RL {depth} --expID 24')
    # normal mlp
    for depth in ['--n_hidden 2', '--n_hidden 3']:
        for first_dim in [256, 1024]:
            commands.append(f'python main.py --policy SAC --automatic_entropy_tuning --env {env} {depth} --first_dim {first_dim} --expID 23')
    # fourier features
    for depth in ['--n_hidden 2']:
        for type in ['--train_B --expID 25', '--concatenate_fourier --train_B --expID 26']:
            fourier_dim = 1024
            for sigma in [0.01, 0.001]:
                commands.append(f'python main.py --policy SAC --automatic_entropy_tuning --env {env} --network_class FourierMLP --sigma {sigma} --fourier_dim {fourier_dim} {depth} {type}')
    count = 0
    for command in commands:
        # gpus = list(range(8,10))
        gpus = [2]
        for seed in [10, 20, 30]:
            print(f'CUDA_VISIBLE_DEVICES={gpus[count]} {command} --seed {seed} &')
            count = (count + 1) % len(gpus)
